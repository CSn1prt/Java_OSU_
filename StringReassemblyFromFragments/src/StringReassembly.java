...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String str1Part = str1.substring(0, str1.length() - overlap);
        String strTotal = str1Part + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        while (strSet.size() > 0) {
            String apart = strSet.removeAny();
            if (str.indexOf(apart) < 0 && apart.indexOf(str) < 0) {
                count++;
            }
            temp.add(apart);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> textSet = new Set1L<>();
        while (!input.atEOS()) {
            String received = input.nextLine();
            if (textSet.size() > 0) {
                addToSetAvoidingSubstrings(textSet, received);

            } else {
                textSet.add(received);
            }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // P.S. out.println(text.replaceAll("~", "\n")); doesn't work here

        for (int pos = 0; pos < text.length(); pos++) {
            if (text.charAt(pos) == '~') {
                out.println();
            } else {
                out.print(text.charAt(pos));
            }
        }
