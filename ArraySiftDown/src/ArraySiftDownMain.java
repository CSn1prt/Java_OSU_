...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        int rightI = top + 1;
        int root = array[top];
        int temp1 = array[leftI];
        int temp2 = array[rightI];

        if (temp1 < temp2) {
            array[top] = temp1;
            array[leftI] = root;
            siftDown(array, top, leftI);
        } else {
            array[top] = temp2;
            array[rightI] = root;
            siftDown(array, top, rightI);

        }
    }
    //when there's left tree only, if the root is smaller than min of left, swap it.
    //when there;s left & right tree, swap root with the min of left & right tree.
    //left tree root = 2 * top + 1
    //there can be left tree w/o right tree, but can't be
