...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!file.atEOS()) {
            ...
            while (pos < str.length()) {
                if (str.charAt(pos) == ',') {
                    letters = str.substring(0, pos);
                    numbers = Integer
                            .parseInt(str.substring(pos + 1, str.length()));
                }

                priceMap.add(letters, numbers);
            }
        }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int cents = price - dollars;
        output.println(dollars + " dollars " + cents + " cents.");
