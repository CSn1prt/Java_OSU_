...
public final class ABCDGuesser1 {
    ...
    private static double getPositiveDoubleNotOne(SimpleReader in,
        ...
        do {
            ...
            while (Double.parseDouble(response) == 1.0) {
                out.println(
                        "It should be a positive real number not equal to 1.0 "
                                + "(Enter a new one): ");
                response = in.nextLine();
            }
            foo = Double.parseDouble(response);
    ...
    public static void main(String[] args) {
        ...
        int i = 0;
        while (i < numTemplate.length) {
            a = numTemplate[i];
            i++;
            int j = 0;
            while (j < numTemplate.length) {
                b = numTemplate[j];
                j++;
                int k = 0;
                while (k < numTemplate.length) {
                    c = numTemplate[k];
                    k++;
                    int l = 0;
                    while (l < numTemplate.length) {
                        d = numTemplate[l];
                        l++;
                        estimateNow = Math.pow(w, a) * Math.pow(x, b)
                                * Math.pow(y, c) * Math.pow(z, d);
                        if (Math.abs((mu - estimateNow) / mu) < Math
                                .abs((mu - estimateFinal)) / mu) {
        ...
        out.println(
                "The value calculated (within a fraction of 1% relative error.): "
                        + estimateFinal);
        out.println("error: " + hundred * Math.abs(mu - estimateFinal) / mu);
