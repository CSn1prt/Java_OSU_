...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            ...
            NaturalNumber remain = new NaturalNumber2(n.divide(m));
            // Created new natural number remain to hold value of n mod m

            reduceToGCD(m, remain);
            // Recursion used

            n.transferFrom(m);
            // transfer m to n in order to update n and clear m
        }
        // If loop checks if the value of m is 0.
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber even = new NaturalNumber2(2);

        /*
         * return statement.
         */
        return n.divide(even).isZero();
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        // TODO - fill in body
        //P.S. might have to use recursion
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //Copy and check the values of w, n.
        NaturalNumber two = new NaturalNumber2(2);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // TODO - fill in body
        int total = 0;
        NaturalNumber firstTest = new NaturalNumber2(n);
        //first test will be with 5 numbers
        int test = firstTest.divideBy10();
        NaturalNumber three = new NaturalNumber2(5);
        NaturalNumber secondTest = new NaturalNumber2(n);
        //second test will be with 3 numbers
        NaturalNumber two = new NaturalNumber2(2);

        if (!n.equals(three)) {
            while (!secondTest.isZero()) {
                int num = secondTest.divideBy10();
                total = total + num;
            }
        } else {
            total = 5;
        ...
        if (total % 3 == 0) {
            result = false;
            //Checking if there's multiple of 3
        } else if (n.compareTo(three) <= 0) {
            result = true;
            //2, 3 are prime number (and 1 is unit number)
        } else if (isEven(n)) {
            result = false;
            // if can be divided by 2, not a prime number
            //
        } else if (test == 5 || test == 0) {
            result = firstTest.isZero();
        } else {
            int nonPrime = 0;
            int max = 0;
            NaturalNumber divisionTest = new NaturalNumber2(n);
            divisionTest.divide(two);
            NaturalNumber hundred = new NaturalNumber2(100);
            if (divisionTest.compareTo(hundred) > 0) {
                max = 100;
            } else {
                max = divisionTest.toInt();
            }
            for (int i = 0; i < max; i++) {
                NaturalNumber rand = new NaturalNumber2(randomNumber(n));
                NaturalNumber one = new NaturalNumber2(1);
                NaturalNumber nMinus = new NaturalNumber2(n);
                NaturalNumber nClone = new NaturalNumber2(n);
                nMinus.decrement();
                ...
                if (rand.equals(one)) {//fix
                    rand.increment();
                }
                if (rand.compareTo(nClone) == 0) {
                    rand.decrement();
                }

                if (isWitnessToCompositeness(rand, n)) {
                    nonPrime++;
                    //checking numbers that are not prime numbers.
                }

            }
            //This for loop checks 100 random numbers
