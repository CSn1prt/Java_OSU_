...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 stat = (Statement2) s;
        StatementLabel rot = this.rep.disassemble(tree);
        tree.add(pos, stat.rep);
        stat.createNewRep();
        this.rep.assemble(rot, tree);
    }
    ...
    public final Statement removeFromBlock(int pos) {
         ...
         * code.
         */
        Statement2 stat = this.newInstance();
        Sequence<Tree<StatementLabel>> tree = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(tree);

        stat.rep = tree.remove(pos);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        // TODO - fill in body
        Sequence<Tree<StatementLabel>> tree = this.rep.newSequenceOfTree();
        Statement2 tempSt1 = (Statement2) s1;
        Statement2 tempSt2 = (Statement2) s2;

        StatementLabel lab = new StatementLabel(Kind.IF_ELSE, c);

        tree.add(0, tempSt1.rep);
        tree.add(1, tempSt2.rep);
        this.rep.assemble(lab, tree);

        tempSt1.createNewRep();
        tempSt2.createNewRep();
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        StatementLabel label = this.rep.disassemble(tree);

        tempSt1.rep = tree.remove(0);
        tempSt2.rep = tree.remove(0);
        this.createNewRep();
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        StatementLabel lab = new StatementLabel(Kind.WHILE, c);
        tree.add(0, temp.rep);
    ...
    public final void assembleCall(String inst) {
        ...
        // TODO - fill in body
        this.rep.assemble(new StatementLabel(Kind.CALL, inst),
    ...
    public final String disassembleCall() {
        ...
        // TODO - fill in body
        // return instruction and go back to original
        StatementLabel label = this.rep
                .disassemble(this.rep.newSequenceOfTree());
        String response = label.instruction;
        this.createNewRep();
