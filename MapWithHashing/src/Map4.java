 ...
 * </pre>
 *
 * @authors Seunghyun Nam & Group 17
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        //Since the way mod() works is different when a < 0, it's in
        //separated case
    ...
    public final void add(K key, V value) {
        ...
        int number = key.hashCode();
        int place = mod(number, this.hashTable.length);
        this.hashTable[place].add(key, value);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        this.size = this.size - 1;
        return this.hashTable[place].remove(key);
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        Pair<K, V> template = null;
        for (int i = 0; i < length; i++) {
            if (this.hashTable[i].size() != 0) {
                template = this.hashTable[i].removeAny();
                i = length;
            }
        }
        this.size = this.size - 1;
    ...
    public final V value(K key) {
        ...
        int number = key.hashCode();
        int place = mod(number, this.hashTable.length);
        V value = this.hashTable[place].value(key);
        return value;
    }
    ...
    public final boolean hasKey(K key) {
        ...
        int number = key.hashCode();
        int place = mod(number, this.hashTable.length);
        boolean yes = this.hashTable[place].hasKey(key);
